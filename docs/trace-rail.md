...
手机胶囊、抽屉标题和侧栏当前节点使用实际区域名称，如项目筛选、资料缺口、信息来源、演示或分享。归档区显示当前日期，其他区域显示分区名。
...
- 手机左拖／右拖以横纵位移比例 1.2 区分方向；横向位移实时控制浮层位置，纵向实时滚动列表并高亮手指附近的候选节点，滑过节点即沿用既有 jump 实时定位正文，松手写入最终路由；取消手势保留预览位置但不写入路由。浮层不阻挡正文，正文 pointerdown 会关闭它，点击日期和 Escape 同样收起。
- 首页手机横向卡片采用原生滚动、proximity snap，80vw 卡宽及 14px 间距露出下一张；关闭手机自动轮播以免干扰触摸惯性。桌面维持原有 3D 排列，拖动时卡片同步偏移，松手换卡。

## 轻量滑选

手机展开宽度 126px，去掉大底板和关闭按钮，保留区域标题、节点与键盘 Escape。轻点入口展开；按下入口立即展开并滑选。超过 8px 区分拖动，边缘停留继续滚动，移动期间不执行节点点击；松手仅提交高亮候选。桌面按住节点也支持实时定位，空白处拖动仍用于浏览列表。手动交互期间暂停 ScrollSpy 内部滚动，结束后 1000ms 恢复。


## 当前位置入口

手机和空间不足的桌面收起为当前位置：分区名或 YYYY·MM·DD，归档入口显示归档信息，不再固定显示组件名称。按下入口立即展开并进入滑选；候选随指针移动高亮，正文实时定位，松手确认并收起。列表内鼠标拖动仍可用于浏览，滚轮、触控板及轻点节点保留。入口、浮层、轨迹及悬停态不使用背景和模糊底板。页面底部补充最后分区识别，避免信息来源无法到达顶部观察线时显示旧日期。

宽度至少 1201px 且高度至少 741px 时桌面轨迹常驻展开，松手及操作正文均保持展开。滑选期间冻结内部月份和跟随滚动，正文同步定位但不新增历史记录；松手仅记录最终节点。滚轮保留独立列表滚动。

日期树仅在正文处于归档区时展开，打开导航或宽屏常驻不再展开日期树。离开归档区立即收纳为归档入口；进入归档后仅当前年份展开月份、当前月份展开日期，其余分支收起。点击年份或月份沿用原有定位，手动交互结束后同步展开目标分支。

行业简报增加周报收纳层：年 → 月 → 周报 → 日期。以已发布周报日期作为周期结束边界，跨月日报归入周报所在年月并显示月日；晚于最新周报的日报归入待汇总。仅当前周报展开，其他功能页不增加此层。筛选保留原始周报边界，周报被筛掉时入口定位该组剩余日期。
