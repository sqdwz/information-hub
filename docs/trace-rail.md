...
手机展开宽度 126px，去掉大底板和关闭按钮，保留区域标题、节点与键盘 Escape。轻点入口展开；按下入口立即展开并滑选。超过 8px 区分拖动，边缘停留继续滚动，移动期间不执行节点点击；松手仅提交高亮候选。桌面拖动仍只浏览、不提交跳转。手动交互期间暂停 ScrollSpy 内部滚动，结束后 1000ms 恢复。


## 当前位置入口

桌面和手机统一收起为当前位置：分区名或 YYYY·MM·DD，归档入口显示归档信息，不再固定显示组件名称。按下入口立即展开并进入滑选；候选随指针移动高亮，松手立即定位并收起。列表内鼠标拖动仍可用于浏览，滚轮、触控板及轻点节点保留。入口、浮层、轨迹及悬停态不使用背景和模糊底板。页面底部补充最后分区识别，避免信息来源无法到达顶部观察线时显示旧日期。
